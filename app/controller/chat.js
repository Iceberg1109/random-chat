...
module.exports = {
  ...
  OnCreateNewPair: function(socket, cur_id) {
    ...
    if(paired) {
      ...
      socket.emit("ON_PAIRFAILED", null);
    }
  },
  ...
  //On Report the partner
  OnReportUser: function(socket, data) {
    console.log("Report");
    var ip = "";
    // Find the pair where current user is joined, remove current pair and add it to sockets array
    for (let element of pairs) {
    ...
    if (ip != "") {
      console.log(ip, data.detail);
      var temp = ip.split(':');
      var ipv4 = temp[temp.length - 1];
      var query = "SELECT * FROM users WHERE ip='" + ipv4 + "'";
      console.log(query);
      con.query(query, function (err, result, fields) {
        if (err) throw err;
        if (result.length != 0) {
          var reports = JSON.parse(result[0].reports);
          reports.push(data.detail);

          var sql = "UPDATE users SET reports = '" + JSON.stringify(reports) +"' WHERE ip='" + ipv4 + "'";
